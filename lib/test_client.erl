...
% forces students not to hardcode "shire"
...
new_client(Nick, GUIName) ->
    ...
    Pid = genserver:start(
        ClientAtom,
        client:initial_state(Nick, list_to_atom(GUIName), ?SERVERATOM),
        fun client:handle/2
    ),
    {Pid, Nick, ClientAtom}.
...
join_nonresponding_server_test() ->
    ...
    % blocking server
    Pid = genserver:start(?SERVERATOM, {}, fun(St, _Msg) ->
        timer:sleep(100000),
        {dead, St}
    end),
    assert("server startup", is_pid(Pid)),
...
nick_change_hack_test() ->
    ...
    assert_error(
        to_string(ClientAtom1) ++ " sends message on " ++ Channel1, Result, user_not_joined
    ).
...
% per channel
-define(CONC_3_USERS, 3).
% per user
-define(CONC_3_MSGS, 2).
...
message_throughput_test() ->
    % sent to clients
    NRecvs = ?CONC_3_CHANS * ?CONC_3_USERS * (?CONC_3_USERS - 1) * ?CONC_3_MSGS,
